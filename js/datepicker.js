import { SELECTORS, MONTH_NAMES, CONFIG } from "./constants.js";

const handleDatepickerInput = (container) => {
    const deadLineInputGroup = container.querySelector(SELECTORS.taskFormGroupDeadline);
    if (!deadLineInputGroup) return;

    const deadLineInput = deadLineInputGroup.querySelector(SELECTORS.taskFormDatepicker);
    if (deadLineInput) {
...
const handleDatepicker = (container, input) => {
    ...
    const renderCalendar = (year, month) => {
        ...
        thead.innerHTML = `
            <tr class="datepicker__header">
                <th class="datepicker__title" colspan="5">${MONTH_NAMES[realMonth]} ${year}</th>
                <th>
        ...
        const handlePickerClick = (e) => {
            ...
            if (!target) return;
            if (target.closest(SELECTORS.datepicker) || target.closest(SELECTORS.taskFormDatepicker)) return;
    ...
    const existingPicker = container.querySelector(SELECTORS.datepicker);
...
export { handleDatepickerInput, handleDatepicker };
